...
import { FixedLinkedList } from "../src/FixedLinkedList";
...
describe("FixedLinked-List Tests", () => {
  ...
  it('should create an empty list #2', () => {
    ...
    let list = new FixedLinkedList<number>();
    expect(list.length).to.equal(0);
    expect(list.size).to.equal(30);
    expect(list.head).to.be.null;
    expect(list.tail).to.be.null;
  });

  it('should create a FixedLinkedList with a single value', () => {
    const list = new FixedLinkedList<number>(5);
    ...
    expect(list.head).to.equal(0);
    expect(list.tail).to.equal(4);
  });

  it('should create a FixedLinkedList with mutiple initial values', () => {
    let values: number[] = [4, 5, 6]
    let list = new FixedLinkedList<number>(3, ...values);
  ...
  it('should support iterable protocol', () => {
    let values: number[] = [4, 5, 6]
    let list = new FixedLinkedList<number>(values.length, ...values);
    let index = 0;
  ...
  it('should allow "any" type', () => {
    ...
    let list = new FixedLinkedList<any>(values.length, ...values);
    expect(list.length).to.equal(3);
    expect(list.head).to.equal(4);
    expect(list.tail).to.equal('hello');
  });

  it('should allow "any" type and return error if theres no size provided', () => {
    let values: any[] = [{ alphabet: 'abc'}, { hello: 'world' }, 'hello']
    let list = new FixedLinkedList<any>(...values);
    expect(list.size).to.equal(3);
    expect(list.length).to.equal(3);
    expect(list.head).to.deep.eq({ alphabet: 'abc'});
    expect(list.tail).to.equal('hello');
  });
  ...
  it('should allow custom types', () => {
    ...
    let list = new FixedLinkedList<Foo>(4, foo1, foo2, foo3, foo4);
  ...
  it('should prevent duplicates when prepending custom types', () => {
    ...
    let list = new FixedLinkedList<Foo>(foo1, foo2, foo3, foo4);
    expect(list.length).to.equal(4);
    expect(list.head).to.equal(foo1);
    expect(list.tail).to.equal(foo4);
  ...
  it('should return the node by nth index', () => {
    ...
    let list = new FixedLinkedList<string>(...values);
    expect(list.length).to.equal(3);
  ...
  it('should return null if list is empty', () => {
    let list = new FixedLinkedList<string>();
    expect(list.length).to.equal(0);
